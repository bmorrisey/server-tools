...
| Deploys | `server-tools deploy <app> <tag>`: pull a prebuilt image from a registry (nothing compiles on the box), re-point the stack, prove the named services are running that exact image, health-verify, and roll back to the previous tag automatically if it does not come up. An app built as several images (a frontend and a backend, say) moves on one tag: all of them are pulled before anything is written, each is verified against its own services, and a release where only half the images moved is rolled back rather than waved through by the health check. Building from a git checkout on the box is still supported for single-app hosts |
